...
export const Lockershow= ({ user, userID }) => {
  ...
  async function addLock(id){
    try{
      db.collection("Locations").doc(id).collection("Lockers").add({
  ...
  return (
    ...
    {locations.map(location => (
        <div className='card' key={location.LocationID}>
          ...
          <button className='locker-add'onClick={()=>setvisible2(true)}>Add lockers</button>
          <Model3 isOpen={visible2} onRequestClose={()=>setvisible2(false)}className='model1box'>
            <h1>Add Locker</h1><br/>
            <p className='count-remainder'>You can only add {location.LocationCount} number of lockers to this location.</p>
            ...
            <input type="text" className='addlocker-form-control' required
                    ref={stateRef} />
                <br /><br/>
            <button type="submit" className='lockeraddbutton' onClick={()=>addLock(location.LocationID)}>Add Locker</button>
            {error && <span className='error-msg'>{error}</span>}
          </form>
          <button onClick={()=>setvisible2(false)} className="locker-cancelbutton">Cancel</button>
          </Model3>

          <Link to="ResDashboard"><button className='locker-go'>Show lockers</button></Link>
